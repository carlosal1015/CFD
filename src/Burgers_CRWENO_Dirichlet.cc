...
void CRWENO5_Dirichlet()
{
  ...
  std::ofstream outfile("CRWENO5.dat");
  if (outfile.is_open()) {
    for (int i = 0; i < nx + 1; i++) {
      outfile << u[nt][i] << " ";
    }
...
void rhs_CR(int nx, double dx, std::vector<double> u, std::vector<double> &r)
{
...
void wenoL_CR(int nx, std::vector<double> u, std::vector<double> &uL)
{
  std::vector<double> a(nx, 0);
  std::vector<double> b(nx, 0);
  std::vector<double> c(nx, 0);
  ...
  std::vector<double> returnValue_for_a(6, 0);
  returnValue_for_a = wcL_CR(v1, v2, v3, v4, v5);
  a[i] = returnValue_for_a[0];
  b[i] = returnValue_for_a[1];
  c[i] = returnValue_for_a[2];
  r[i] = returnValue_for_a[3] * u[i - 1] + returnValue_for_a[4] * u[i] +
         returnValue_for_a[5] * u[i + 1];
...
std::vector<double> wcL_CR(double v1, double v2, double v3, double v4,
                           double v5)
{
  double eps = 1.0e-6;
  std::vector<double> returnValue(6, 0);
  ...
  double c1 = 2.0e-1 / (std::pow(eps + s1, 2));
  double c2 = 5.0e-1 / (std::pow(eps + s2, 2));
                           ...
                           double v5)
{
  ...
  double c2 = 0.5 / (std::pow(eps + s2, 2));
  double c3 = 0.2 / (std::pow(eps + s3, 2));
