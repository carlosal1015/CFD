...
void rhs_roe(int nx, double dx, double gamma,
             ...
             std::vector<std::vector<double>> &r)
{
  ...
  std::vector<double> alp(nx, 0);
  std::vector<double> alpha(nx, 0);
  for (int i = 0; i < nx; i++)
  {
          double p = (gamma - 1.0) * (q[2][i] - 0.5 * q[1][i] * q[1][i] /
  q[0][i]); f[0][i] = q[1][i]; f[1][i] = q[1][i] * q[1][i] / q[0][i] + p;
          f[2][i] = q[1][i] * q[2][i] / q[0][i] + p * q[1][i] / q[0][i];
  }
  for (int i = 0; i < nx; i++)
  {
          a = std::sqrt(gamma * ((gamma - 1) * (q[2][i] - 0.5 * q[1][i] *
  q[1][i] / q[0][i])) / q[0][i]); alp[i] = abs(q[1][i] / q[0][i]) + a;
  }

  for (int i = 2; i < nx - 3; i++)
  {
          alpha[i] = std::max(alp[i - 2], std::max(alp[i - 1], std::max(alp[i],
  std::max(alp[i + 1], alp[i + 2]))));
  }
  alpha[0] = alp[0];
  alpha[1] = alp[1];
  alpha[nx - 2] = alp[nx - 2];
...
void flux_roe(int nx, double gamma, std::vector<std::vector<double>> q,
...
std::vector<std::vector<double>> wenoL_roe(int nx,
                                           std::vector<std::vector<double>> q)
{
  std::vector<std::vector<double>> f(3, std::vector<double>(nx + 1, 0));
  for (int m = 0; m < 3; m++) {
...
std::vector<std::vector<double>> wenoR_roe(int nx,
                                           std::vector<std::vector<double>> q)
{
  std::vector<std::vector<double>> f(3, std::vector<double>(nx + 1, 0));
  for (int m = 0; m < 3; m++) {
