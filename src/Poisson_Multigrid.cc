...
void Poisson_MG()
{
  ...
  for (iter_count = 0; iter_count < max_iter && rms > exp_rms; iter_count++) {
    // call relaxation on fine grid and compute the numerical solution for fixed
    // number of iteration
                 ...
                 std::vector<std::vector<double>> &ec)
{
  for (int i = 1; i < nyc; i++) {
    for (int j = 1; j < nxc; j++) {
      ...
      double grid = 2.0 * (r[2 * i][2 * j + 1] + r[2 * i][2 * j - 1] +
                           r[2 * i + 1][2 * j] + r[2 * i - 1][2 * j]);
      double corner = 1.0 * (r[2 * i + 1][2 * j + 1] + r[2 * i + 1][2 * j - 1] +
    ...
    for (int i = 1; i < ny; i++) {
      for (int j = 1; j < nx; j++) {
        rt[i][j] = f[i][j] -
                   ...
                   (un[i][j + 1] - 2 * un[i][j] + un[i][j - 1]) / dx / dx;
        un[i][j] += rt[i][j] / den;
      }
    }
  }
